...
public class GoalRepository implements GoalDao{
    ...
    public int insertGoal(int id, Goal goal) {
        String sql="INSERT INTO GOAL(TITLE,DESCRIPTION,AMOUNT,GOALDATE)VALUES(?,?,?,?)";
        return jdbcTemplate.update(sql,new Object[]{goal.getTitle(),goal.getDescription(),goal.getAmount(),goal.getGoalDate()});
    }

    @Override
    public int updateGoal(int id, Goal goal) {
        String sql="UPDATE GOAL SET TITLE=?,DESCRIPTION=?,AMOUNT=?,GOALDATE=? WHERE ID=?";
        return jdbcTemplate.update(sql,new Object[]{goal.getTitle(),goal.getDescription(),goal.getAmount(),goal.getGoalDate(),id});
    }
